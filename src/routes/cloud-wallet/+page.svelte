<script lang="ts">
  ...
  >
    ...
    {#if form?.success}
      ...
      <p class="pt-2">Logged in! Your total satoshi is: {form?.balance} and your paymail id is: {form?.paymail}</p>
    {/if}
  </div>



                            <!-- The following is for the payment to others -->
  <div
    class="mt-10 pt-10 w-full max-w-xl p-12 mx-auto rounded-lg shadow-xl dark:bg-white/10 bg-white/30 ring-1 ring-gray-900/5 backdrop-blur-lg"
  >
    ...
    {#if form?.success}
      <!-- this message is ephemeral; it exists because the page was rendered in
             ...
             <p class="pt-2 text-green-600 font-semibold">
                Payment Successful!
                <a href={`https://whatsonchain.com/tx/${form?.payresponse}`} class="text-blue-600 hover:text-blue-800 visited:text-purple-600">
